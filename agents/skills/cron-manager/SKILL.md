...
2. **Schema Enforcement:** Always provide all keys. Use `null` for inactive parameters.

3. **Hydration:** For `is_random` tasks, set flags to `True` but leave targets `null`—the CronService will seed specific times.

4. **Verification:** Before adding, list existing jobs to prevent duplicates.

5. **Live Truth:** When checking status, prioritize lock files and heartbeat over registry metadata.
...
2. Add the job with appropriate schedule:
   ```bash
