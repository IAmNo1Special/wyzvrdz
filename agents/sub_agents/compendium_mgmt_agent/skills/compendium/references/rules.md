# Markdown Rules & Obsidian Standards

## Obsidian Optimized Checklist

- **Frontmatter**: Category, Tags, and `updated: 'YYYY-MM-DD'`.
- **Heading**: Single `# Title` at top. No other H1s.
- **Links**: Always use `[[Wikilinks]]`.
- **Relations**: Use `Key:: [[Value]]`.
- **Spacing**: ONE blank line around all headings and lists.
- **Formatting**: Run `scripts/format_compendium_files.py` after edits.
...
Note: Emphasis within a word is restricted to `asterisk` in order to avoid
unwanted emphasis for words containing internal underscores like\_\_this\_\_one.
...
1. Append an incrementing integer (as needed for uniqueness)
1. [URI-encode][encodeuricomponent] the result
...
- `prohibited_texts`: Prohibited link texts (`string[]`, default `["click here","here","link","more"]`)
